...
long double interpreter::group_expression(const std::unordered_map<std::string, long double>& vars) {
	#ifdef DEBUG
	std::cout << "GROUP_EXPRESSION" << std::endl;
	#endif
	if(current_token.get_type() == L_PAREN) {
		eat(L_PAREN);
		long double res = additive_expression(vars);
		eat(R_PAREN);
		return res;
	}
	long double numres = numeric_expression(vars);
	return numres;
}

long double interpreter::exponential_expression(const std::unordered_map<std::string, long double>& vars) {
	long double g1 = group_expression(vars);
	if(current_token.get_type() == EXP) {
		eat(EXP);
		long double g2 = exponential_expression(vars);
		return pow(g1, g2);
	}
	return g1;
}

long double interpreter::numeric_expression(const std::unordered_map<std::string, long double>& vars) {
	#ifdef DEBUG
	std::cout << "NUMERIC_EXPRESSION: " << current_token << std::endl;
	#endif
	if(current_token.get_type() == DOUBLE) {
		long double val = current_token.get_double_value();
		eat(DOUBLE);
		return val;
	} else if(current_token.get_type() == MINUS) {
		eat(MINUS);
		long double val = exponential_expression(vars);
		return -val;
		...
		eat(SIN);
		eat(L_PAREN);
		long double v = additive_expression(vars);
		eat(R_PAREN);
		return sin(v);
...
long double interpreter::additive_expression(const std::unordered_map<std::string, long double>& vars) {
	#ifdef DEBUG
	std::cout << "ADDITIVE_EXPRESSION" << std::endl;
	#endif
	long double res = multiplicative_expression(vars);
...
long double interpreter::multiplicative_expression(const std::unordered_map<std::string, long double>& vars) {
	#ifdef DEBUG
	std::cout << "MULTIPLICATIVE_EXPRESSION" << std::endl;
	#endif
	long double res = exponential_expression(vars);
	...
	while(current_token.get_type() == MUL || current_token.get_type() == DIV) {
		if(current_token.get_type() == MUL) {
			...
			eat(DIV);
			long double tmp = exponential_expression(vars);
			if(tmp == 0) {
...
long double interpreter::interpret(const std::unordered_map<std::string, long double>& vars) {
	token_ptr = 0;
	if(tokens.size() <= 0) {
		return NAN;
	}
